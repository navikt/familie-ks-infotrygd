...
internal class ClientValidatorTest {
    ...
    private fun setupMocks(issuer: String, claimKey: String, claimValue: String) {
        ...
        Mockito.`when`(validationContext.getClaims(issuer)).thenReturn(claims)

        Mockito.`when`(oidcRequestContextHolder.tokenValidationContext).thenReturn(validationContext)
    }
